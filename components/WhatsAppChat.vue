<template>
  <section
    class="fixed bottom-8 right-0 mb-4 mr-2 w-auto flex items-center justify-center z-50 font-montserrat bg-transparent"
  >
    <div class="container mx-auto">
      <div
        class="shrink-0 flex items-center bg-gradient-to-b from-slate-700 to-blue-500 rounded-lg font-montserrat"
      >
        <a
          href="https://api.whatsapp.com/send/?phone=628119999331&text=Brochure%21%0D%0A%0D%0ASaya+mendapatkan+informasi+terkait+%2APerumahan+Pine+Residence+di+OCBD+Bogor%2A+dengan+Cicilan+6+Juta-an+melalui+situs+web+https%3A%2F%2Focbd.co.id%2F+.+Bisakah+Anda+memberikan+informasi+lebih+lanjut+%3F%0D%0A%0D%0ATerima+kasih.&type=phone_number&app_absent=0"
          class="flex items-center space-x-2 my-2 mx-5"
          target="_blank"
        >
          <img
            src="../assets/images/whatsapp.png"
            alt="contact"
            class="w-8 h-8"
          />
          <h3 class="text-white font-montserrat">Hubungi Kami!</h3>
        </a>
      </div>
    </div>
  </section>
</template>

<script>
export default {
  name: "WhatsAppChat",
};
</script>

<style scoped>
/* Tambahkan style tambahan jika diperlukan */
</style>
